...
- DB자료 처리 순서
1. 드라이버 로딩 => JDBC라이버러리를 사용할 수 있도록 메모리로 읽어들이는 작업
...
3. 질의 => SQL문장을 DB서버로 보내서 결과를 얻어온다
        (Statement객체가 PreparedStatement객체를 이용하여 작업한다.)
4. 결과 처리 => 질의 결과를 받아서 원하는 작업을 수행한다
    1)SQL 문장이 'select'문일 경우에는 select한 결과가 ResultSet객체에 저장되어 반환한다.
...
public class JDBCTest01 {
    public static void main(String[] args) {
        ...
        try{
            ...
            //3-3. SQL문장을 DB서버로 보내서 결과로 얻어온다.
            //      (실행할 SQL문장이 'select'문이기 때문에 결과가 ResultSet객체에
            //      저장되어 반환된다.)
            ...
            //  => ResultSet객체 데이터를 가리키는 포인터를 다음번째 레코드로 이동시키고
